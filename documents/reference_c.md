...
ただし、SAPPOROBDD++ を使用している場合（SAPPOROBDD_PLUS_PLUS マクロが定義されている場合）は、B_32 マクロが定義されていない場合に 1 を返し、定義されている場合に 0 を返す。SAPPOROBDD++ では 64 ビット版が既定であり、32 ビット版を使用する場合に B_32 マクロを定義するためである。
...
```
int bddisvalid(bddp f)
```

**本関数は実装されていない。** f が正しい bddp であるかの判定には
SAPPOROBDD の内部データへのアクセスが必要であり、
ワンヘッダライブラリからは行えないためである。
API の互換性のために名前のみが残されており、
