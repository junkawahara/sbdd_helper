...
DDIndex<int> index1(f);              // f を保持しているので正しい

DDIndex<int> index2(getPowerSet(10)); // 誤り。一時オブジェクトは破棄される
```
...
`DDIndex<T>` は内部で確保したメモリを所有するため、
コピーコンストラクタと代入演算子を禁止している（private 宣言のみで定義がない）。
以下のようなコードはコンパイルエラーとなる。

```
DDIndex<int> index1(f);
DDIndex<int> index2 = index1; // コンパイルエラー
index2 = index1;              // コンパイルエラー
```

関数の引数として渡す場合は参照（`DDIndex<int>&` または `const DDIndex<int>&`）を用いる。
同じ BDD/ZBDD に対する別のインデックスが必要な場合は、
コピーではなく改めてコンストラクタで作成する。
...
内部で確保しているメモリを解放する。
既に clear() が呼ばれている場合でも二重解放は起こらない。
...
内部で確保しているメモリ（ノードインデックス、各ノードに関連付けられた
データ、要素数の計算結果など）を明示的に解放する。
デストラクタの呼び出しを待たずにメモリを解放したい場合に用いる。
（Python バインディングなど、デストラクタの呼び出しタイミングを
制御できない環境を想定している。）

clear() は何度呼び出してもよい（2回目以降は何もしない）。

clear() を呼び出した後も、DDIndex の各メンバ関数を呼び出すことはできる。
その場合、対象の BDD/ZBDD が空であるかのように振る舞い、
count() や size() は 0 を、getZBDD() や getKSetsZBDD() は ZBDD(0) を、
getSet() や usedVar() や sampleRandomly() は空集合を返す。
また、begin() と end()（および各種イテレータの開始・終了）は等しくなる。

clear() は各ノードに関連付けられたデータも解放する。そのため、clear() を
呼び出す前に取得していた `DDNode<T>` の value メンバや、getStorageRef() で
得ていた参照は、clear() の後は無効な参照となる。これらを読み書きしても
コンパイルエラーや実行時エラーにはならないが、解放済みのメモリへの
アクセスとなるため、行ってはいけない。clear() の後に再びノードのデータを
使う場合は、改めてインデックスを構築する必要がある。
...
weights の大きさは、f に現れる変数番号の最大値より大きくなければならない。
これを満たさない場合は、エラーメッセージを出力してプログラムを終了（exit(1)）する。
重みの和の計算が llint の範囲を超える場合も、
エラーメッセージを出力してプログラムを終了（exit(1)）する
（GMP を利用できる場合は、任意桁で計算できる getSumMP の利用を検討すること）。
本メンバ関数は内部にカウント用の情報を記憶させる。
